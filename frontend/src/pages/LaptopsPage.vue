                        ...
                        <v-col v-for="laptop in filteredLaptops" :key="laptop.id" cols="12" sm="6" md="6" lg="4" xl="3">
                            <v-card class="product-card h-100 d-flex flex-column rounded-xl" elevation="0" @click="goToProduct(laptop.id)" style="cursor: pointer;">
                                ...
                                <v-card-actions class="pa-4 pt-0 d-flex justify-space-between align-end">
                                    ...
                                    <v-btn icon color="#059669" variant="tonal" class="cart-btn rounded-lg" @click.stop="handleAddToCart(laptop)" title="Adaugă în coș">
...
<script setup>
    ...
    const dynamicMaxPrice = computed(() => {
        if (allLaptops.value.length === 0) {
            return 10000;
        }

        const currentCatProducts = allLaptops.value.filter(p => p.category === activeCategory.value);
