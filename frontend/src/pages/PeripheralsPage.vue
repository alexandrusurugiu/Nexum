                        ...
                        <v-col v-for="peripheral in filteredPeripherals" :key="peripheral.id" cols="12" sm="6" md="6" lg="4" xl="3">
                            <v-card class="product-card h-100 d-flex flex-column rounded-xl" elevation="0" @click="goToProduct(peripheral.id)" style="cursor: pointer;">
                                ...
                                <v-card-actions class="pa-4 pt-0 d-flex justify-space-between align-end">
                                    ...
                                    <v-btn icon color="#059669" variant="tonal" class="cart-btn rounded-lg" @click.stop="handleAddToCart(peripheral)" title="Adaugă în coș">
...
<script setup>
    ...
    const dynamicMaxPrice = computed(() => {
        if (allPeripherals.value.length === 0) {
            return 10000;
        }

        const currentCatProducts = allPeripherals.value.filter(p => p.category === activeCategory.value);
