...
export const useAuthStore = defineStore('auth', () => {
    ...
    const toggle2FA = async (isEnabled) => {
        try {
            await axios.put(`${import.meta.env.VITE_API_URL}/server/auth/${user.value.id}/toggle-2fa`, { isEnabled });
    ...
    const updateProfile = async (updates) => {
        isLoading.value = true;
        try {
            const response = await axios.put(`${import.meta.env.VITE_API_URL}/server/auth/${user.value.id}/update`, updates);
            if (response.data.success) {
                user.value = response.data.user;
                localStorage.setItem('nexum_user', JSON.stringify(user.value));
                return true;
